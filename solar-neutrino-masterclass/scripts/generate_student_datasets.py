    ...
    pd.DataFrame(
        {
            ...
            "epsilon_borexino_like": efficiency(T, 0.20, 0.04),
            "background_sk_shape": np.exp(-np.clip(T - 3.0, 0.0, None) / 3.0),
            "background_borexino_shape": 0.65 * np.exp(-T / 0.45) + 0.35 * np.exp(-0.5 * ((T - 1.2) / 0.25) ** 2),
        }
    ).to_csv(OUT / "detector_response.csv", index=False)

    pd.DataFrame(
        [
            {"detector": "SK-like", "parameter": "threshold_MeV", "value": 4.5, "unit": "MeV"},
            {"detector": "SK-like", "parameter": "threshold_width_MeV", "value": 0.35, "unit": "MeV"},
            {"detector": "SK-like", "parameter": "exposure", "value": 100.0, "unit": "kton year water"},
            {"detector": "Borexino-like", "parameter": "threshold_MeV", "value": 0.20, "unit": "MeV"},
            {"detector": "Borexino-like", "parameter": "threshold_width_MeV", "value": 0.04, "unit": "MeV"},
            {"detector": "Borexino-like", "parameter": "exposure", "value": 100.0, "unit": "ton year scintillator-like"},
        ]
    ).to_csv(OUT / "detector_models.csv", index=False)
...
def write_predictions(E: np.ndarray, spectra: dict[str, np.ndarray], day: np.ndarray, night: np.ndarray) -> None:
    ...
    borexino_bins = np.arange(0.10, 2.05, 0.05)

    sk_exposure = ELECTRONS_PER_KTON_WATER * 100.0 * SECONDS_PER_YEAR
    borexino_exposure = ELECTRONS_PER_KTON_WATER * 0.10 * SECONDS_PER_YEAR
    ...
    sk.to_csv(OUT / "sk_like_event_predictions.csv", index=False)

    borexino = expected_counts(E, spectra, borexino_bins, borexino_exposure, 0.20, 0.04, day)
    borexino.to_csv(OUT / "borexino_like_event_predictions.csv", index=False)
    ...
    bx_total = borexino.groupby(["T_low_MeV", "T_high_MeV", "T_center_MeV"], as_index=False)[["events_day"]].sum()
    bx_total["background"] = 18.0 * (
        0.65 * np.exp(-bx_total["T_center_MeV"] / 0.45)
        + 0.35 * np.exp(-0.5 * ((bx_total["T_center_MeV"] - 1.2) / 0.25) ** 2)
    )
    bx_total["expected"] = bx_total["events_day"] + bx_total["background"]
    bx_total["observed"] = rng.poisson(bx_total["expected"])
    bx_total["seed"] = 20260628
    bx_total.to_csv(OUT / "pseudo_data_borexino.csv", index=False)
